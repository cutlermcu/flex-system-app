...
// List all flex dates and create new ones

import { createServerClient } from '@/lib/supabase/server';
import { createAdminClient } from '@/lib/supabase/admin';
import { NextResponse } from 'next/server';

// GET - List all flex dates with stats
export async function GET() {
  try {
    ...
    // Get all flex dates
    const { data: flexDates, error } = await adminClient
      .from('flex_dates')
      .select('*')
    ...
    // Get stats for each flex date
    const flexDatesWithStats = await Promise.all(
    ...
    return NextResponse.json({ flexDates: flexDatesWithStats });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    const { date, flex_type, duration_minutes, selection_deadline } = body;
    ...
    // Check if flex date already exists
